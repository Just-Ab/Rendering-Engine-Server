...
#include <Rendering/Shader.h> 
        ...
        bool Shader::setupShader(unsigned int*shader,GLenum type,const char* source){
        ...
        bool Shader::setupProgram(unsigned int*program,unsigned int**shader,uint8_t count){
            int result;
        ...
        void Shader::setMat4(const char*uniformName,glm::mat4 value){
            GLint location = glGetUniformLocation(shaderProgram,uniformName);
            if (location>=0){
        ...
        void Shader::setVec3(const char*uniformName,glm::vec3 value){
            GLint location = glGetUniformLocation(shaderProgram,uniformName);
            if (location>=0){
        ...
        void Shader::setVertexAttributes(Attribute _attribute){
            vertexAttributes.resize(1);
            vertexAttributes[0] = _attribute;
        }
        void Shader::setVertexAttributes(std::vector<Attribute> _attributes){
            vertexAttributes = _attributes;
        }

        void Shader::setInstanceAttributes(Attribute _attribute){
            instanceAttributes.resize(1);
            instanceAttributes[0] = _attribute;
        }
        void Shader::setInstanceAttributes(std::vector<Attribute> _attributes){
            instanceAttributes = _attributes;
        }

        std::vector<Attribute>& Shader::getVertexAttributes(){
            return vertexAttributes;
        }

        std::vector<Attribute>& Shader::getInstanceAttributes(){
            return instanceAttributes;
        }

        unsigned int Shader::getUniformBlockIndex(const char* blockName){
                return glGetUniformBlockIndex(shaderProgram,blockName);
            }

        void Shader::bindUniformBlock(const char* blockName,unsigned int index){
